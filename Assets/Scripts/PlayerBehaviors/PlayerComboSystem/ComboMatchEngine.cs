    ...
    public class ComboMatchEngine 
    {
        ...
        private void InitializeComboRules()
        {
            ...
            _comboRules = new List<IComboRule>()
            {
                ...
                new JabCross(_comboActionCommandFactory),
                new DodgeBack(_comboActionCommandFactory),
                new DodgeLeft(_comboActionCommandFactory),
                new DodgeRight(_comboActionCommandFactory),
                new ChainPunches(_comboActionCommandFactory),
        ...
        internal bool CanStartCombo(Queue<ICombatCommand> comboSequence)
        {
        ...
        public void CheckAndExecuteCombo()
        {
            ...
            ICombatCommand comboCommand = CheckSequenceForCombo(comboSequence);
           
            if (comboCommand != null && ( _actionQueueManager.attackStats.ReturnCurrentStamina() >= _actionQueueManager.attackStats.StaminaCosts[comboCommand.ToString()]))
            {
                Debug.Log(comboCommand.ToString());
        ...
        private bool DoesFirstActionStartCombo(Queue<ICombatCommand> comboSequence)
        {
        ...
        private ICombatCommand CheckSequenceForCombo(Queue<ICombatCommand> comboSequence)
        {
        ...
        private IEnumerable<ICombatCommand> GetSubsequence(Queue<ICombatCommand> comboSequence, int startIndex)
        {
        ...
        private void ExecuteComboAction(ICombatCommand comboCommand)
        {
